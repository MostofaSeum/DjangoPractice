...
test.describe("Checkout Page", () => {
  ...
  async function setupCheckoutSession(
    page: any,
    options: {
    ...
    if (authenticated) {
      ...
      if (token) {
        try {
          const syncRes = await page.request.get(
            "http://127.0.0.1:8000/store/carts/sync/",
            {
              headers: { Authorization: `JWT ${token}` },
            },
          );
          if (syncRes.ok()) {
            const userCart = await syncRes.json();
            if (userCart?.id) {
              await page.request.delete(
                `http://127.0.0.1:8000/store/carts/${userCart.id}/`,
                {
                  headers: { Authorization: `JWT ${token}` },
                },
              );
    ...
    if (!emptyCart) {
      try {
        const cartRes = await page.request.post(
  ...
  test.describe("1. Authentication Gate & Empty Cart State", () => {
    ...
    test("displays empty cart state when cart has no items and browse products button works", async ({
      ...
      const emptyHeading = page.getByRole("heading", {
        name: /Your Cart is Empty|Cart is Empty|কার্ট খালি/i,
      });
      await expect(emptyHeading).toBeVisible({ timeout: 15000 });

      const browseBtn = page.getByRole("link", {
        name: /Browse Products|পণ্য দেখুন/i,
      });
  ...
  test.describe("2. Breadcrumb Navigation", () => {
    ...
    test("renders breadcrumbs and allows navigation back to Home or Cart", async ({
      page,
    }) => {
  ...
  test.describe("3. Phone Number Input & Validation", () => {
    ...
    test("prefills customer phone number or allows entering 11-digit numbers", async ({
      page,
    }) => {
  ...
  test.describe("4. Saved Addresses & Delivery Zone Auto-Detection", () => {
    ...
    test("loads saved addresses, preselects default address, and populates shipping address textarea", async ({
      page,
    }) => {
    ...
    test("selecting custom address clears textarea, and editing textarea auto-switches dropdown to custom", async ({
      page,
    }) => {
      const addressDropdown = page.locator("form select").first();
      const addressTextarea = page.locator("form textarea").first();
      ...
      if (await addressDropdown.isVisible()) {
        ...
        expect(await addressTextarea.inputValue()).toBe(
          "Flat 5A, Banani, Dhaka",
        );

        // Select the first saved address again
        const firstOptionVal = await addressDropdown
          .locator("option")
          .first()
          .getAttribute("value");
    ...
    test("switching between saved addresses auto-toggles delivery zone based on address city", async ({
      page,
    }) => {
      const addressDropdown = page.locator("form select").first();
      if (await addressDropdown.isVisible()) {
        const outsideOption = addressDropdown
          .locator("option")
          .filter({ hasText: /Outside Dhaka/i })
          .first();
  ...
  test.describe("5. Delivery Zone Selection & Pricing", () => {
    ...
    test("displays estimated delivery days for Inside and Outside Dhaka", async ({
      page,
    }) => {
      ...
      const insideCard = insideRadio.locator(
        'xpath=ancestor::div[contains(@class, "rounded-2xl")][1]',
      );
      const outsideCard = outsideRadio.locator(
        'xpath=ancestor::div[contains(@class, "rounded-2xl")][1]',
      );
    ...
    test("toggling delivery zone updates delivery charge and total amount in order summary", async ({
      page,
    }) => {
      const insideRadio = page.locator("#delivery_inside");
      const outsideRadio = page.locator("#delivery_outside");
      ...
      const insideTotal = parseFloat(
        (await totalEl.innerText()).replace(/[^\d.]/g, ""),
      );
  ...
  test.describe("6. Payment Methods & Gateway Validation", () => {
    ...
    test("payment gateways render based on active settings (COD, bKash, Nagad, VibeCoin)", async ({
      page,
    }) => {
    ...
    test("bKash requires sender phone number and transaction ID", async ({
      page,
    }) => {
      const bkashCard = page
        .locator("#pay_bkash")
        .locator('xpath=ancestor::div[contains(@class, "rounded-2xl")][1]');
      await bkashCard.click();

      // Transaction Phone and TrxID inputs must appear in BkashPaymentUI
      const trxPhoneInput = page
        .getByPlaceholder(/01XXXXXXXXX|Sender Number|বিকাশ নাম্বার/i)
        .or(page.locator('input[type="tel"]').nth(1));
      const trxIdInput = page
        .getByPlaceholder(/TrxID|Transaction ID|ট্রানজেকশন আইডি/i)
        .or(page.locator('input[placeholder*="TrxID"]'));

      await expect(trxPhoneInput).toBeVisible({ timeout: 10000 });
      await expect(trxIdInput).toBeVisible({ timeout: 10000 });

      const confirmBtn = page.getByRole("button", {
        name: /Confirm Order|Place Order|অর্ডার সম্পন্ন করুন/i,
      });
      ...
      if (!(await addressTextarea.inputValue()))
        await addressTextarea.fill("House 1, Road 2, Dhaka");

      // Now button is enabled
      await expect(confirmBtn).toBeEnabled();
    });

    test("Nagad requires sender phone number and transaction ID", async ({
      page,
    }) => {
      const nagadCard = page
        .locator("#pay_nagad")
        .locator('xpath=ancestor::div[contains(@class, "rounded-2xl")][1]');
      ...
      const trxPhoneInput = page
        .getByPlaceholder(/01XXXXXXXXX|Sender Number|নগদ নাম্বার/i)
        .or(page.locator('input[type="tel"]').nth(1));
      const trxIdInput = page
        .getByPlaceholder(/TrxID|Transaction ID|ট্রানজেকশন আইডি/i)
        .or(page.locator('input[placeholder*="TrxID"]'));
    ...
    test("VibeCoin option indicates balance status and shows instructions when eligible", async ({
      page,
    }) => {
      const vibeCoinRadio = page.locator("#pay_vibecoin");
      const vibeCoinCard = vibeCoinRadio.locator(
        'xpath=ancestor::div[contains(@class, "rounded-2xl")][1]',
      );
      ...
      if (!isDisabled) {
        ...
        const instructionCard = page
          .locator("text=VibeCoin Payment Ready, text=কয়েন কর্তন করা হবে")
          .or(page.getByText(/VibeCoin Payment Ready/i));
        ...
        await expect(vibeCoinCard).toContainText(
          /Blocked|Insufficient|Not enough|ব্যালেন্স অপর্যাপ্ত/i,
        );
        await vibeCoinCard.click();
  ...
  test.describe("7. Coupons & Order Summary Calculations", () => {
    test("renders applied coupon discount in order summary with correct deduction", async ({
      page,
    }) => {
      ...
      const discountAmountEl = page
        .locator(".space-y-3")
        .locator("div")
        .filter({ hasText: /SUMMER20/i })
        .locator("span")
        .nth(1);
      await expect(discountAmountEl).toContainText("-");
    });

    test("automatically removes coupon if cart contains no eligible products", async ({
      page,
    }) => {
      ...
      const subtotalEl = page
        .locator(".space-y-3 span.font-bold.text-foreground")
        .first();
      ...
      const couponRow = page
        .locator(".space-y-3")
        .getByText(/INELIGIBLE_COUPON/i);
      await expect(couponRow).toHaveCount(0);
    });

    test("order summary calculates subtotal matching cart page items value", async ({
      page,
    }) => {
    ...
    test("intercepts invalid or expired coupon on order submission", async ({
      page,
    }) => {
      await setupCheckoutSession(page, {
        productId: 1,
      ...
      const couponAlert = page
        .locator(".swal2-popup")
        .or(page.getByText(/Coupon No Longer Valid/i));
      await expect(couponAlert.first()).toBeVisible({ timeout: 15000 });

      // Coupon should be removed from localStorage
      const savedCoupon = await page.evaluate(() =>
  ...
  test.describe("8. Order Placement, Cart Reset & Error Handling", () => {
    test("cannot submit order when phone or address is empty", async ({
      page,
    }) => {
      await setupCheckoutSession(page);
    ...
    test("submitting valid Cash on Delivery order confirms order, clears cart, and redirects to profile", async ({
      page,
    }) => {
      await setupCheckoutSession(page);
      ...
      const codCard = codRadio.locator(
        'xpath=ancestor::div[contains(@class, "rounded-2xl")][1]',
      );
      await codCard.click();
      ...
      const storedCoupon = await page.evaluate(() =>
        localStorage.getItem("applied_coupon"),
      );
    ...
    test("submitting valid order paid via VibeCoin creates order and updates balance", async ({
      page,
    }) => {
      await setupCheckoutSession(page);
    ...
    test("handles backend order failure gracefully with error alert and re-enables button", async ({
      page,
    }) => {
      await setupCheckoutSession(page);
      ...
      await page.route("**/store/orders/", async (route) => {
        await route.fulfill({
          ...
          contentType: "application/json",
          body: JSON.stringify({
            error: "Item out of stock or warehouse unavailable.",
          }),
        });
      });
      ...
      const errorPopup = page
        .locator(".swal2-popup")
        .or(page.getByText(/Order Placement Failed|সমস্যা দেখা দিয়েছে/i));
